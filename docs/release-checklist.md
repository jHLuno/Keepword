...
- [ ] Review the migration set. This release includes `0009_check_page_callback_tokens`, `0010_suggestion_events`, `0011_preserve_suggestion_event_history`, `0012_multilingual_replies`, `0013_group_suggestion_edit_sessions`, and `0014_check_commitment_picker_tokens`; all are forward-only. `0009` replaces the callback-token constraint and `0011` drops the actor-membership foreign key, so the schema changes are not all purely additive; `0012` adds `NOT NULL` language columns with defaults (`chats.language='auto'`, `commitment_suggestions.language='en'`, `commitments.language='en'`), while `0013` and `0014` add scoped session/token tables. No migration deletes existing data.
...
- [ ] Create and edit a detected group suggestion as its source author. Verify the old card loses its buttons, only that author can reply to the group instruction, and the revised card still needs confirmation. A different user or chat must not apply the edit.
- [ ] From a private overdue reminder, choose `Перенести срок`; verify the old private card loses its buttons, a Russian future date such as `сегодня в 22:00` uses the source-chat timezone, and a past or invalid date keeps the private retry flow open. No overdue detail or new deadline may be posted to the group.
- [ ] Create at least six active commitments for one onboarded user across two staging chats. Run `/check`; verify source-chat labels, no more than five picker items on a page, next/previous navigation, one selected detail card at a time, and that `Готово`, `Перенести срок`, and `Есть блокер` alter only that user's intended commitment while consuming the used card controls.
...
- [ ] In a group, run bare `/settings`; verify it shows that group's current mode, language, timezone, and digest time plus the four group commands. A current admin can update those values only in that group; a non-admin cannot mutate them. In private, verify `/settings on|off [number]` changes only that user's notification delivery and directs group configuration back to the relevant group.
