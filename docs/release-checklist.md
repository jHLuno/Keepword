...
- [ ] Copy a `/check` **page-navigation** (`check_page`) callback to a different Telegram account, then reuse it from the original account. Verify both attempts are rejected and no commitment changes.
